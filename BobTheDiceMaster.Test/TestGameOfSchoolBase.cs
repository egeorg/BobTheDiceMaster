...
namespace BobTheDiceMaster.Test
{
  ...
  public abstract class TestGameOfSchoolWithDiceBase<T> where T : GameOfSchoolWithDiceBase
  {
    protected T game;
    protected Mock<IDie> diceMock = new Mock<IDie>();

    protected IDie GetMockedDice() => diceMock.Object;

    [Fact]
    public void TestCorrectDiceMockIsUsed()
    {
      int[] firstRollResult = new[] { 1, 2, 2, 6, 6 };
      diceMock.Setup(x => x.Roll(5)).Returns(firstRollResult);
      Assert.Equal(game.GenerateRoll(), new DiceRollDistinct(firstRollResult));

      game.Reset();

      int[] secondRollResult = new[] { 6, 6, 6, 6, 6 };
      diceMock.Setup(x => x.Roll(5)).Returns(secondRollResult);
      Assert.Equal(game.GenerateRoll(), new DiceRollDistinct(secondRollResult));
    }
